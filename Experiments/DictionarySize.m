...
t_grid   	= mNRMSE_grid;
t_gllim  	= mNRMSE_gllim; t_gllim_learn = nan(size(nb_signals,1), size(nb_signals,2));
t_nn        = mNRMSE_gllim; t_nn_learn = t_gllim_learn;
...
for n = 1:size(nb_signals,1)
    ...
    for f = 1:size(nb_signals,2)
        ...
        t_gllim_learn(n,f) = Estim.Regression.learning_time;
        
        if dbdl_computation == 1
            [Estim,NeuralNet] = AnalyzeMRImages([],Dico_DBL,'DB-DL',NeuralNet_);
            t_nn_learn(n,f) = Estim.Regression.learning_time;
        end
...
t(:,2,:,:)      = t_gllim;
t(:,3,:,:)      = t_nn;
t_learn(2,:,:)  = t_gllim_learn;
t_learn(3,:,:)  = t_nn_learn;
...
disp(' ')
disp('Average RMSE obtained with the DB-DL method are lower than the one obtained with the DBM method:')
disp([num2str(...
100* wmean(reshape((squeeze(mRMSE(1:end-2,1,[3],:)) - squeeze(mRMSE(1:end-2,3,[3],:)))...
    ./ squeeze(mRMSE(1:end-2,1,[3],:)),1,[]), wei)...
, 3) ' +/- ' num2str(...
100* var(reshape((squeeze(mRMSE(1:end-2,1,[3],:)) - squeeze(mRMSE(1:end-2,3,[3],:)))...
    ./ squeeze(mRMSE(1:end-2,1,[3],:)),1,[]), wei).^.5 ...
, 3) ' % for 5 parameters'])
disp([num2str(...
100* wmean(reshape((squeeze(mRMSE(1:end-2,1,[5],:)) - squeeze(mRMSE(1:end-2,3,[5],:)))...
    ./ squeeze(mRMSE(1:end-2,1,[5],:)),1,[]), wei)...
, 3) ' +/- ' num2str(...
100* var(reshape((squeeze(mRMSE(1:end-2,1,[5],:)) - squeeze(mRMSE(1:end-2,3,[5],:)))...
    ./ squeeze(mRMSE(1:end-2,1,[5],:)),1,[]), wei).^.5...
, 3) ' % for 7 parameters'])


disp(' ')
disp('Average RMSE obtained with the DB-SL method are lower than the one obtained with the DBM method:')
disp([num2str(...
100* wmean(reshape((squeeze(mRMSE(1:end-2,1,[3],:)) - squeeze(mRMSE(1:end-2,2,[3],:)))...
    ./ squeeze(mRMSE(1:end-2,1,[3],:)),1,[]), wei)...
, 3) ' +/- ' num2str(...
100* var(reshape((squeeze(mRMSE(1:end-2,1,[3],:)) - squeeze(mRMSE(1:end-2,2,[3],:)))...
    ./ squeeze(mRMSE(1:end-2,1,[3],:)),1,[]), wei).^.5 ...
, 3) ' % for 5 parameters'])
disp([num2str(...
100* wmean(reshape((squeeze(mRMSE(1:end-2,1,[5],:)) - squeeze(mRMSE(1:end-2,2,[5],:)))...
    ./ squeeze(mRMSE(1:end-2,1,[5],:)),1,[]), wei)...
, 3) ' +/- ' num2str(...
100* var(reshape((squeeze(mRMSE(1:end-2,1,[5],:)) - squeeze(mRMSE(1:end-2,2,[5],:)))...
    ./ squeeze(mRMSE(1:end-2,1,[5],:)),1,[]), wei).^.5...
, 3) ' % for 7 parameters'])

disp(' ')
disp('DB-DL compared to DB-SL across all conditions')
disp([num2str(...
100* wmean(reshape(squeeze(mean((squeeze(mRMSE(1:end-2,2,[3 5],:)) - squeeze(mRMSE(1:end-2,3,[3 5],:)))...
    ./ squeeze(mRMSE(1:end-2,2,[3 5],:)),2)),1,[]) , wei)...
, 3) ' +/- ' num2str(...
100* var(reshape(squeeze(mean((squeeze(mRMSE(1:end-2,2,[3 5],:)) - squeeze(mRMSE(1:end-2,3,[3 5],:)))...
    ./ squeeze(mRMSE(1:end-2,2,[3 5],:)),2)),1,[]) , wei).^.5...
, 3)])


disp(' ')
disp(['For the DBM method the difference between the biggest and the smallest dictionaries (7 parameters) is ' ...
...
disp(['For the DB-DL method the difference between the biggest and the smallest dictionaries (7 parameters) is ' ...
    num2str(100* wmean(reshape((mRMSE(1:end-2,3,5,1) - mRMSE(1:end-2,3,5,end)) ./ mRMSE(1:end-2,3,5,1),1,[]), w'),3) ...
    ' +/- ' num2str(100* var(reshape((mRMSE(1:end-2,3,5,1) - mRMSE(1:end-2,3,5,end)) ./ mRMSE(1:end-2,3,5,1),1,[]), w').^.5, 3)])
disp(['For the DB-SL method the difference between the biggest and the smallest dictionaries (7 parameters) is ' ...
...
disp(['For the DB-DL method the difference between the 2 highest dictionaries (7 parameters) is ' ...
    num2str(100* wmean(reshape((mRMSE(1:end-2,3,5,end-1) - mRMSE(1:end-2,3,5,end)) ./ mRMSE(1:end-2,3,5,end-1),1,[]), w'),3) ...
    ' +/- ' num2str(100* var(reshape((mRMSE(1:end-2,3,5,end-1) - mRMSE(1:end-2,3,5,end)) ./ mRMSE(1:end-2,3,5,end-1),1,[]), w').^.5, 3)])
disp(['For the DB-SL method the difference between the 2 highest dictionaries (7 parameters) is ' ...
...
disp(' ')
disp('Altogether, compared to the DBM method, the DB-SL method reduces the RMSE by:')
disp([num2str(...
100* wmean((squeeze(mRMSE(1:end-2,1,[3],end)) - squeeze(mRMSE(1:end-2,2,[3],2)))...
    ./ squeeze(mRMSE(1:end-2,1,[3],end)), w)...
, 3) ' +/- ' num2str(...
100* var((squeeze(mRMSE(1:end-2,1,[3],end)) - squeeze(mRMSE(1:end-2,2,[3],2)))...
    ./ squeeze(mRMSE(1:end-2,1,[3],end)),w).^.5...
, 3) ' % for 5 parameters'])

disp([num2str(...
100* wmean((squeeze(mRMSE(1:end-2,1,[5],end)) - squeeze(mRMSE(1:end-2,2,[5],1)))...
    ./ squeeze(mRMSE(1:end-2,1,[5],end)), w)...
, 3) ' +/- ' num2str(...
100* var((squeeze(mRMSE(1:end-2,1,[5],end)) - squeeze(mRMSE(1:end-2,2,[5],1)))...
    ./ squeeze(mRMSE(1:end-2,1,[5],end)),w).^.5...
, 3) ' % for 7 parameters'])
...
disp(' ')
disp('Altogether, DB-SL versus DB-DL:')
vv = snr_levels <= 30;
disp([num2str(...
100* nanmean(reshape((squeeze(mRMSE(vv,3,:,:)) - squeeze(mRMSE(vv,2,:,:)))...
    ./ squeeze(mRMSE(vv,3,:,:)), 1,[]))...
, 3) ' +/- ' num2str(...
100* nanvar(reshape((squeeze(mRMSE(vv,3,:,:)) - squeeze(mRMSE(vv,2,:,:)))...
    ./ squeeze(mRMSE(vv,3,:,:)), 1,[])).^.5...
, 3) ' % (SNR < 30)'])
vv = snr_levels > 60; vv(end) = 0;
disp([num2str(...
100* nanmean(reshape((squeeze(mRMSE(vv,2,:,:)) - squeeze(mRMSE(vv,3,:,:)))...
    ./ squeeze(mRMSE(vv,2,:,:)), 1,[]))...
, 3) ' +/- ' num2str(...
100* nanvar(reshape((squeeze(mRMSE(vv,2,:,:)) - squeeze(mRMSE(vv,3,:,:)))...
    ./ squeeze(mRMSE(vv,2,:,:)), 1,[])).^.5...
, 3) ' % (SNR > 60)'])
